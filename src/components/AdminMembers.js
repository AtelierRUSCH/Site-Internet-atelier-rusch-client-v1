...
const AdminMembers = ({ members, thanks }) => {
  const displayMembers = members.map(el => (
        ...
        <button
          ...
          onClick={() => {
            if (
              ...
              api.deleteMember(el.id).then(() => window.location.reload())
            } else {
            }
          }}
        >
  ...
  const displayThanks = thanks.map(el => (
    <div className="AdminCardHalfWidth">
      <div className="currentText"> {el.name} </div>
      <div className="EditButtonsContainer">
        ...
        <button
          ...
          onClick={() => {
            if (
              ...
              api.deleteThank(el.id).then(() => window.location.reload())
            } else {
            }
          }}
        >
  ...
  return (
    <div className="GlobalContainer">
      ...
      <div className="AdminCardHalfWidthContainer">{displayMembers}</div>

      <Link to="thanks/new" onClick={() => window.scrollTo(0, 0)}>
        <div style={{ marginTop: '6rem' }} className="ButtonCreateElement">
          <MdAdd className="ReactIconAdd" />
          Créer un nouveau remerciement
        </div>
      </Link>
      <div className="AdminTitles yellow">Remerciements :</div>
      <div className="AdminCardHalfWidthContainer">{displayThanks}</div>
    </div>
  )
}
